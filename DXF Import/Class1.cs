	...
	public class Class1
	{
		...
		static void LoadDXF(ref List<Wire> DXFAssembly, string FilePath)
		{
			...
			foreach (netDxf.Entities.Line item in dxfFile.Lines)
			{
				...
				//DXFDrawing.Bodies.Aldd(Body.CreateLine(new Vector3(item.StartPoint.X, item.StartPoint.Y, item.StartPoint.Z), new Vector3(item.EndPoint.X, item.EndPoint.Y, item.EndPoint.Z)));
			}
		...
		static void PathGenerator(ref Part DXFDrawing, ref RsPathProcedure PlasmaProgram, double Kerf, double VerticalOffset)
		{
			...
			foreach (Body Bodyitem in DXFDrawing.Bodies)
			{
				foreach (Wire WireItem in Bodyitem.Wires)
				{
					...
					PathPoses[1] = PathPoses[1].Multiply(new Matrix4(new Vector3(Kerf * 2, Kerf * 3, -.01)));
					PathMoveTypes.Insert(1, RsPointType.ToPoint);

					PathPoses.Insert(2, PathPoses[2]);
					PathPoses[2] = PathPoses[2].Multiply(new Matrix4(new Vector3(Kerf * 2, Kerf * 3, -VerticalOffset)));
					...
					for (int i = 0; i < PathPoses.Count; i++)
					{
						...
						double AngleDelta=0;
						double AngleDeltaStart=0;
						double AngleDeltaEnd=0;
						...
						AngleDeltaStart = PathPoses[i].EulerZYX.z;
						if (AngleDeltaStart < 0) AngleDeltaStart += 2 * Math.PI;

						if (i+1<PathPoses.Count)
						{
							AngleDeltaEnd = PathPoses[i + 1].EulerZYX.z;
							if (AngleDeltaEnd < 0) AngleDeltaEnd += 2 * Math.PI;
							AngleDelta = AngleDeltaStart- AngleDeltaEnd;
						}
						else
						{
							AngleDeltaEnd = PathPoses[i - 1].EulerZYX.z;
						...
						TempTarget.Frame.Matrix = TempTarget.Frame.Matrix.Multiply(new Matrix4(new Vector3(Kerf, Math.Sin(AngleDelta/2)*Math.Sqrt(2) * Kerf, 0)));
